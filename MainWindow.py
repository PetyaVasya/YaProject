...
class MainWindow(QMainWindow):
    ...
    def open_parsers_list(self):
        ...
        self.parsers_list = ParsersList(self.parsers, self)
        self.setCentralWidget(self.parsers_list)
        self.parsers_list.createNewButton.clicked.connect(self.open_parser_edit)
        for ind, parser in enumerate(self.parsers_list.parsers_elements):
            parser.clicked.connect(lambda: self.open_parser_edit(self.parsers[ind]))
        self.parsers_list.delete_buttons.buttonClicked.connect(self.delete_parser)

    def delete_parser(self, btn):
        reply = QMessageBox.question(self, "Delete parser", "Do you want delete parser?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            id = self.parsers_list.delete_buttons.id(btn)
            self.parsers_list.delete_buttons.removeButton(btn)
            self.parsers_list.parsers_elements.pop(id).setParent(None)
            self.parsers.pop(id)
    ...
    def finalEditAction(self, btn):
        if btn.text() == "OK":
            ...
            self.parsers.append(Parser(self.parser_edit.name_edit.toPlainText()))
            self.setWindowTitle(self.parser_edit.name_edit.toPlainText())
        else:
            self.open_parsers_list()
